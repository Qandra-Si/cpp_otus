...
Практическая часть домашнего задания с реализацией и использованием "разреженной бесконечной матрицы" находится в файле `src/homework15_operators/main.cpp`. Вывод данных из матрицы выполнен с помощью proxy-класса `matrix_stub_t`, который реализуя copy/direct initialization, а также перегружая оператор тип() по умолчанию, позволяет читать данные хранимые в матрице, а также при обращении к assignment оператору выполнять редактирование данных в матрице.
