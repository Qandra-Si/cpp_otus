...
Выполнение требования "...матрица должна хранить только занятые элементы..." исключая хранение элементов со значениями по умолчанию продемонстрировано с помощью теста, использующего `fake_heap_allocator_t` аллокатор, см. тест `matrix_allocator` в файле `src/core/test/test_matrix.cpp`. Тест инициализиует крошечное кол-во памяти в фейковой куче и выполняет множество размещений элементов со значениями по умолчанию, проверяя факт того, что память на размещение элементов не была потрачена.

Сборка исходных кодом домашнего задания:
