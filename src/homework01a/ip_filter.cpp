...
int main(int, char* [])
{
  ...
  try
  {
    ...
    std::qsort(
      ...
      endian.v_addr[0] ? little_endian_cmp : big_endian_cmp
    );

    // выводим адреса из пула в отсортированном виде
    for (const auto & ipv4 : ip_pool)
    {
      std::cout << (int)ipv4.v_addr[0] << '.' << (int)ipv4.v_addr[1] << '.' << (int)ipv4.v_addr[2] << '.' << (int)ipv4.v_addr[3] << std::endl;
    }
    // выводим адреса из пула, первый октет которых равен 1
    for (const auto & ipv4 : ip_pool)
    {
      if (ipv4.v_addr[0] != 1) continue;
      std::cout << (int)ipv4.v_addr[0] << '.' << (int)ipv4.v_addr[1] << '.' << (int)ipv4.v_addr[2] << '.' << (int)ipv4.v_addr[3] << std::endl;
    }
    // выводим адреса из пула, первый октет которых равен 46, а второй 70
    for (const auto & ipv4 : ip_pool)
    {
      if ((ipv4.v_addr[0] != 46) || (ipv4.v_addr[1] != 70)) continue;
      std::cout << (int)ipv4.v_addr[0] << '.' << (int)ipv4.v_addr[1] << '.' << (int)ipv4.v_addr[2] << '.' << (int)ipv4.v_addr[3] << std::endl;
    }
    // выводим адреса из пула, первый октет которых равен 46, а второй 70
    for (const auto & ipv4 : ip_pool)
    {
      if ((ipv4.v_addr[0] != 46) && (ipv4.v_addr[1] != 46) && (ipv4.v_addr[2] != 46) && (ipv4.v_addr[3] != 46)) continue;
      std::cout << (int)ipv4.v_addr[0] << '.' << (int)ipv4.v_addr[1] << '.' << (int)ipv4.v_addr[2] << '.' << (int)ipv4.v_addr[3] << std::endl;
    }
  }
